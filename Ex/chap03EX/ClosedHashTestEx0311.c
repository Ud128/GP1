// 演習3-11　オープンアドレス法によるハッシュの利用例（キーは氏名）
...
int main( void ) {
	...
	do {
		...
		switch( menu = SelectMenu() ) {
			...
			case DELETE:	/*--- データの削除 ---*/	//変えた所★
			x = ScanMember( "削除" , MEMBER_NAME );		//MEMBER_NO --> MEMBER_NAME★
			result = Remove( &hash , &x );
			if( result == 1 )
				printf( "\aエラー：その番号のデータは存在しません。\n" );
			break;

			case SEARCH:	/*--- データの探索 ---*/	//変えた所★
			x = ScanMember( "探索" , MEMBER_NAME );		//MEMBER_NO --> MEMBER_NAME★
			temp = Search( &hash , &x );
			if( temp == NULL )
				printf( "\a探索に失敗しました。\n" );
			else {
