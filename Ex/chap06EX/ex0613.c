// 演習6-13　クイックソート（非再帰版：プッシュ・ポップ・分割の詳細を表示）
...
void quick( int a[] , int left , int right ) {
	...
	//最初のLR
	printf( "a[%d]～a[%d]を分割する問題をスタックにプッシュします。\n" , left , right );
	...
	while( !IsEmpty( &lstack ) ) {
		...
		//スタックから何が取り出されたのかを出力
		printf( "スタックから分割する問題をポップで取り出しました。a[%d]～a[%d]を分割します。\n" , left , right );
		...
		if( left < pr ) {
			...
			//分割されてた結果がどのようにため込まれているのかをPrintでチェック
			printf( "a[%d]～a[%d]を分割する問題をスタックにプッシュします。\n" , left , pr );
			printf( "Lstack:" );   Print( &lstack );
			printf( "Rstack:" );   Print( &rstack );
		}
